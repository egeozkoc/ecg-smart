...
def find_thresholds_from_val(y_true, y_prob_pos):
    ...
    rule_out_thresh = thresholds[rule_out_idx[-1]] if len(rule_out_idx) > 0 else None
    ...
    rule_in_thresh = thresholds[rule_in_idx[0]] if len(rule_in_idx) > 0 else None
...
def compute_metrics(y_true, y_prob_pos, threshold):
    ...
    npv = precision_score(y_true, 1 - preds, zero_division=0)
...
def plot_roc_curve(y_true, y_pred, model_name, save_path):
...
def plot_confusion_matrix(cm, model_name, save_path):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                xticklabels=['No OMI', 'OMI'], 
                yticklabels=['No OMI', 'OMI'])
...
def main():
    ...
    model_path = 'models_median_beats/ecgsmartnet_omi_2025-07-28-22-17-23.pt'

    # Device
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

    # Load splits
    x_val_np, y_val_np = load_split_data('preprocessed_median_beats', 'val', 'omi')
    x_test_np, y_test_np = load_split_data('preprocessed_median_beats', 'test', 'omi')
